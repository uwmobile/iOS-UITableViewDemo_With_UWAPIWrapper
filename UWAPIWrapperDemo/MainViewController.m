...
- (IBAction)onRequestClicked:(id)sender {
    ...
    if (!_api_key_txt_field.text || [_api_key_txt_field.text isEqualToString:@""]) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Need your api key" message:@"Please go to api.uwaterloo.ca to request a key" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
        [alert show];
    }
    ...
    [[UWAPIWrapper sharedInstance] requestParsedJSONResponseWithMethodName:@"Holidays" QueryIfNeeded:query APIKey:_api_key_txt_field.text completionBlock:^(id parsedJSONObject) {
